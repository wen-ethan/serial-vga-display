...
parser is complete and passing a self-checking testbench in simulation. The font
ROM is generated from a public-domain 8x8 font, checked glyph by glyph in
simulation, and confirmed to synthesize into block RAM rather than logic. The
rest of the render path (character RAM, and the character generator that scans
it out) is not written yet.
...
`tb_Font_ROM` works differently: it prints every glyph it reads as ASCII art, so
the simulation log is the result rather than a waveform.
